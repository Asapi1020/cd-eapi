...
export class RecordUsecase {
	...
	private async fetchServerName(serverIP: string): Promise<string | null> {
		...
		const serverInfo: QueryResult = await steamAPIClient.getServerInfo(serverIP.split(":")[0]).catch((error) => {
	...
	private async getSteamUserMap(steam32IDs: string[]): Promise<Map<string, SteamUser>> {
		const steamIDMap = new Map(steam32IDs.map((id) => [id, this.convertSteam32To64ID(id)]));
		const steam64IDs = Array.from(steamIDMap.values());
		const steamUsers = await new SteamAPIClient().getPlayerSummaries(steam64IDs);
